...
const Chat = () => {
	...
	useEffect(() => {
		//console.log("personR: ", personR.id)
		if (user) {
			scrollToLastMsg();
	...
	const sendText = async () => {
		...
		if (selectedElement === '1') { //выбран паспорт
			...
			// const url_send_msg = `https://api.telegram.org/bot${token_work}/sendMessage?chat_id=${personR.id}&parse_mode=html&text=${temp}`
			...
			if(!file) {
				...
				if (fileType === 'doc') { //(image.slice(-3) === 'gif' || image.slice(-3)==='zip') {
					...
					// 	const url_send_doc = `https://api.telegram.org/bot${token_work}/sendMessage?chat_id=${personR.id}&parse_mode=html&text=${host+image}`
					...
					// 	const url_send_doc = `https://api.telegram.org/bot${token_work}/sendDocument?chat_id=${personR.id}&document=${host+image}`
					// 	//console.log("url_send_doc: ", url_send_doc)
					// 	sendPhotoToTelegram = await $host.get(url_send_doc);
					// } else {
						const url_send_doc = `https://api.telegram.org/bot${token_work}/sendDocument?chat_id=${personR.id}&document=${host+image}`
					...
					// 	setShowErrorFile(true)
					// } else {
						const url_send_photo = `https://api.telegram.org/bot${token_work}/sendPhoto?chat_id=${personR.id}&photo=${host+image}`
